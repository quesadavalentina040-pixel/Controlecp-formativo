    ...
    public function personal_data_load_store(Request $request)
    {
        ...
        if ($validator->fails()) {
            return back()
                ...
                ->with('danger', 'Se ha producido un error.')
                ->withInput();
        }
    
        $file = $request->file('archivo'); // ✅ obtiene el archivo como objeto UploadedFile
        ...
        if (!$readerType) {
            return back()->with('danger', 'El archivo debe tener extensión .xlsx, .xls o .csv.');
        }
    
        // ✅ Lee el archivo directamente
        $array = \Maatwebsite\Excel\Facades\Excel::toArray(new PeopleImport, $file, null, $readerType);
        $datos = $array[0];
        $num = count($datos);
        ...
        foreach ($datos as $d) {
            ...
            $person->telephone1 = (!empty($d[5]) && is_numeric($d[5])) ? $d[5] : null;
    
            if (str_contains($d[6], '@misena')) {
                $person->misena_email = strtolower($d[6]);
            } elseif (str_contains($d[6], '@sena')) {
            ...
            $person->eps_id = 0;
            $person->pension_entity_id=1;
            $person->population_group_id = $pgid;
            $person->save();
        }
    
        return back()->with('message', 'Excel importado correctamente. ' . $num . ' Personas Agregadas/Actualizadas')
        ...
        if($validator->fails()){
            ...
            try {
                ...
                foreach($apprentices_data as $data){
                    ...
                    $person = Person::updateOrCreate(['document_number' => intval($data[1])],[ // Consultar o preparar registro de persona
