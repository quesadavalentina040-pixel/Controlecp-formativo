                                            ...
                                            @foreach ($environment_pus as $epu)
                                                @if ($previous_productive_unit_id !== $epu->productive_unit_id)
                                                    <tr>
                                                        ...
                                                        {{ $epu->productive_unit->name ?? 'N/A' }}
                                                    </td>
                                                @else
                                                    <tr>
                                                @endif
                                                    <td class="text-center">{{ $epu->environment->name ?? 'N/A' }}</td>
                                                    <td class="text-center">
                                                        <a href="#" onclick="event.preventDefault(); if(confirm('¿Estás seguro de que deseas eliminar la asociación de la unidad productiva {{ $epu->productive_unit->name ?? 'N/A' }} y el ambiente {{ $epu->environment->name ?? 'N/A' }}?')) { document.getElementById('delete-form-epu{{ $epu->id }}').submit(); }" data-toggle='tooltip' data-placement="top" title="Eliminar asociación">
